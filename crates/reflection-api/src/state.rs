...
impl AppState {
    ...
    async fn process_job(&self, job_id: Uuid) -> Result<()> {
        ...
        if matches!(job.discovery, DiscoveryMode::Browser | DiscoveryMode::Auto)
            && job.status != JobStatus::CandidateSelected
        {
    ...
    async fn process_candidate(&self, job: &JobRecord, candidate: &MediaCandidate) -> Result<()> {
        ...
        match candidate.kind {
            CandidateKind::Audio => {
                ...
                let output_path = job_dir.join("audio.mp3");
            ...
            CandidateKind::Video | CandidateKind::Manifest => {
                ...
                let output_path = job_dir.join("video.mp4");
                Transcoder::new(self.config.ffmpeg_path.clone())
                    .media_url_to_mp4(&candidate.url, &output_path)
                    .await?;
